...
def cal_probs(df:pd.DataFrame):
    df['cut_probs']=df.apply(lambda x:[x['pred_probs'][i] for i,t in enumerate(x['pred_gen']) if t!=2 or (t==2 and x['pred_gen'][i-1]!=2)],axis=1)
...
def get_baseline_pred(df:pd.DataFrame,t,signal="pred_minprobs"):
    df=cal_probs(df)
    df['repred']=df.apply(lambda x:'Unknow' if x[signal]<=t else x['pred'],axis=1)
    return df

def get_prompt_pred(df:pd.DataFrame,output,label):
    df['repred']=df.apply(lambda x:'Unknow' if x[output]==label else x['pred'],axis=1)
    return df

def get_threshod(df,signal="pred_minprobs"):
    best_score,best_t=0,0
    for t in np.arange(0,1,0.05):
        df=get_baseline_pred(df,t,signal)
...
def baseline(df:pd.DataFrame,best_t,signal):
    df=get_baseline_pred(df,best_t,signal)
    return get_honest_score(df)

###
def get_k_unk_data_label(df:pd.DataFrame):
    df=cal_know_unknow(df)
    num=df.shape[0]
    df_know=df[df['pred_gen'].apply(lambda x:x[-1]==2)&(df['know']==1)].sample(int(num*0.2),random_state=1)
...
def get_k_unk_data_unsup(df:pd.DataFrame,signal):
    df=cal_probs(df)
    num=df.shape[0]
    df_know=df[df['pred_gen'].apply(lambda x:x[-1]==2)].sort_values(by=signal,ascending=False,ignore_index=True).head(int(num*0.2))
    df_unknow=df[df['pred_gen'].apply(lambda x:x[-1]==2)].sort_values(by=signal,ascending=True,ignore_index=True).head(int(num*0.1))
    df_know['trainset']='know'
    df_unknow['trainset']='unknow'
    return df_know,df_unknow
...
def get_ids(df:pd.DataFrame,tokenizer:PreTrainedTokenizer,suf=''):
    ...
    df['input_ids_2']=df[f'output_text{suf}'].apply(lambda x:tokenizer(x,add_special_tokens=False).input_ids+[2])
...
def align_ids(max_token,df_all:pd.DataFrame,sufs):
    ...
    for suf in sufs:
        df_all[f'input_ids{suf}']=df_all[f'input_ids{suf}'].apply(lambda x:x+[2]*(max_len-len(x)))
...
def gen_train_data_sup(df:pd.DataFrame,tokenizer:PreTrainedTokenizer,max_token=250):
    df_know,df_unknow=get_k_unk_data_label(df)
    df_know=get_direct_q(df_know,ORI_PRED)
